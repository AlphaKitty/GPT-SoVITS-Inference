...
from tools.my_infer import get_multi_ref_template, create_speaker_list, single_infer, multi_infer, pre_infer, get_classic_model_list, classic_infer, get_version, get_installed_models, install_model, delete_model, openai_like_infer, openai_like_model_list
...
async def classic_model_list(version: str):
    try:
...
#OpenAI风格的模型列表
@APP.get("/v1/models")
async def openai_compatible_model_list():
    try:
        model_list = openai_like_model_list()
    except Exception as e:
        print(e)
...
async def check_model(version: str):
    try:
        installed_models, msg = get_installed_models(version)
    except Exception as e:
        print(e)
        installed_models = []
        msg = "获取已安装模型失败"
    return {"installed": installed_models, "msg": msg}
